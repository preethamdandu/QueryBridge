...
async function healthTool(_req: IncomingMessage, res: ServerResponse): Promise<void> {
  sendJson(res, 200, {
    result: {
      ok: true,
...
async function handleMcpCall(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (body.toolName !== "health") {
    sendJson(res, 400, { error: "Unsupported toolName" });
    return;
  }

  await healthTool(req, res);
}
...
async function requestHandler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (path === "/health") {
    sendJson(res, 200, { ok: true, service: "query-service" });
    return;
  }
  ...
  if (path === "/mcp/tools/health" || path === "/tools/health") {
    await healthTool(req, res);
    return;
  }
...
async function bootstrap(): Promise<void> {
  ...
  console.log(`query-service ready at http://localhost:${port}`);
}
